...
def crawl_website(start_url: str) -> dict:
    """
    BFS crawl of all internal pages. Returns:
      {
        "pages_visited": [...],
        "text": "...",
        "image_urls": [...],
...
def scrape_restaurant(place_id: str, name: str, website: str, dry_run: bool = False) -> dict:
    ...
    for img_url in result["image_urls"]:
        if downloaded >= MAX_IMAGES:
            break
        dest_path = os.path.join(dest_dir, f"{place_id}-WebsiteScraper-{downloaded}.jpg")
        if _download_image(img_url, dest_path):
            downloaded += 1
...
def run(limit: int | None = None, dry_run: bool = False, force: bool = False):
    ...
    logger.info("Found %d restaurants with scrapeable websites", total)
    ...
    for i, (place_id, name, website) in enumerate(rows, 1):
        ...
        try:
            ...
            if result.get("status") == "ok":
                logger.info("  → %d pages, %d images", result["pages"], result["images"])
            processed += 1
        except Exception as exc:
